...
c      INTEGER FUNCTION LKUPEP (PRLST, POINTR, PRLGTH, ALPHPR, BASE)
C
C
C
C     SPECIFICATIONS FOR PARAMETERS
...
C
c      INTEGER
c     1   I, INDEX (29), MCH (29), MCHLTH, MCHPTR, NOFLAG,
c     2   PRBASE, PRCHR, PRLIM, PRPTR, TMPSUB, WDSCHR, WDSLIM, WDSLTH,
c     3   WDSPTR (29)
C
c      REAL*8
c     1   WDS (29)
c      LOGICAL WDSLOGICAL(8,29)
C
c      INTEGER
c     1   ICHREP
C
...
C
c      REAL*8   ALPHP0
c      DOUBLE PRECISION DIDFEP
C
c      LOGICAL
c     1   ALPHP1 (8)
C
C     FORCE ALPHP1 TO A DOUBLE-WORD BOUNDARY
c      EQUIVALENCE (ALPHP0, ALPHP1 (1))
c      EQUIVALENCE (WDS(1),WDSLOGICAL(1,1))
C
C
C
C
...
c      DATA WDS /
c     1   8HIS        , 8HUSE       , 8HNO        ,
c     2   8HCOMPLEX   , 8HREAL      , 8HHERMITIA  ,
c     3   8HSYMMETRI  , 8HTRIDIAGO  , 8HPOSITIVE  ,
c     4   8HLARGEST   , 8HBALANCE   , 8HORTHOGON  ,
c     5   8HRATQR     , 8HSMALLEST  , 8HELEMENTA  ,
c     6   8HBISECT    , 8HNEGATIVE  , 8HYES       ,
c     7   8HDO        , 8HFULL      , 8HGENERAL   ,
c     8   8HAX=LBX    , 8HABX=LX    , 8HBAX=LX    ,
c     9   8HAZ=WBZ    , 8HABZ=WZ    , 8HBAZ=WZ    ,
c     O   8HPACKED    , 8HUNITARY
c    1   /
cC
c      DATA INDEX /
c     1             +1,           +1,           -1,
...
c  105 CONTINUE
c      LKUPEP = 0
c      PRBASE = PRLST (POINTR)
cC
cC     SAVE THE FIRST EIGHT CHARACTERS OF THE ALPHANUMERIC PARAMETER
cC        IN  ALPHPR  IN CASE AN ERROR MESSAGE MUST BE PRINTED.  THE
cC        CHARACTERS ARE COPIED ONE BYTE AT A TIME TO AVOID SPECIFICATION
cC        EXCEPTIONS WHICH WOULD ARISE ON IBM S/360 IF STRINGS WERE
cC        NOT COMPILED ON A DOUBLE WORD BOUNDARY.  THE FOLLOWING LOOP
Cc        AND ITS RELATED DECLARATIONS AND EQUIVALENCE STATEMENT
cC        CAN BE REPLACED ON OTHER MACHINES BY A SIMPLE ASSIGNMENT TO
cC        ALPHPR.
cC
c      DO 101 I = 1, 8
c         TMPSUB = PRBASE + I - 1
c         ALPHP1 (I) = BASE (TMPSUB)
c  101 CONTINUE
c      ALPHPR = DIDFEP (ALPHP0)
c      PRPTR = 0
cC
c      DO 102 I = 1, WDSLTH
c         WDSPTR (I) = 0
...
c         MCHLTH = MCHPTR
c         MCHPTR = 0
c         PRCHR = ICHREP (BASE (PRBASE), PRPTR, PRLIM)
c         IF (PRCHR .EQ. -1) RETURN
cC
c         DO 103 I = 1, MCHLTH
c            TMPSUB = MCH (I)
c      WDSCHR = ICHREP (WDSLOGICAL(1,TMPSUB), WDSPTR (TMPSUB), WDSLIM)
c            IF (PRCHR .NE. WDSCHR) GO TO 103
c            MCHPTR = MCHPTR + 1
c            MCH (MCHPTR) = MCH (I)
c  103    CONTINUE
c      IF (MCHPTR .GE. 2) GO TO 104
c      IF (MCHPTR .EQ. 0) RETURN
c      TMPSUB = MCH (1)
c      NOFLAG = NOFLAG*INDEX (TMPSUB)
c      LKUPEP = NOFLAG
c      IF (IABS (NOFLAG) .NE. 1) RETURN
c      POINTR = POINTR + 1
c      IF (POINTR .LE. PRLGTH) GO TO 105
c      RETURN
C
c      END
C
