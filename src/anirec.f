...
c
c  f77 -o anirec -O anirec.f lib/*.o
...
c  Modified by H. Yuan 2002
c  1. turn on sv/sh incident waves: 
c     do ity=1,1 to do ity=1,3 around line 352
c  2. used sac header:
c     user5: slowness; user6: zero time (s); baz; delta; cmpaz; cmpinc; 
c     kcmpnm
      ...
      character*80 name,title,xlabel,ylabel(3),xlabel2,name2
      character *4 chead(158)
      character *3 wf(3)
      complex*16 pp,u0,ee,z1,z0,xnu,e1,e2,zla,xl
      complex*16  rt,tt,rt0,trc,pfac,u,resp,zz
      real*4 cc4,zz4,frqq,amn,amx,baseline,dat4,a,ahead,dat44
      common/data/a(40000)
...
c      print *,'either BAZ is determined by the loop count'
c      print *,'or, in interactive mode, BAZ is user-input'
c      print *,'if BAZ=0, the wave is arriving from N'
c      print *,'in anirec.bh? the N component is the -RADIAL'
c      print *,'          and the E component is the TRANSVERSE'
c      print *,'if BAZ=90, the wave is arriving from E'
c      print *,'  in anirec.bh? the N component is the -TRANSVERSE'
c      print *,'and the             E component is the -RADIAL'
c      print *,'if BAZ=180, the wave is arriving from S'
c      print *,'  in anirec.bh? the N component is the RADIAL'
c      print *,'and the             E component is the -TRANSVERSE'
c      print *,'if BAZ=270, the wave is arriving from W'
c      print *,'  in anirec.bh? the N component is the TRANSVERSE'
c      print *,'and the             E component is the RADIAL'
c  version for fully interactive input of baz and phase velocity
      print *,'enter phase velocity of incident wave (km/sec)'
      read(5,*) cc
      if(cc.le.0.d0) go to 950
c  non-dimensionalize cc
      cc=cc*1000./vbar
      print *,'enter back azimuth of incident wave (deg CW from N)'
      read(5,*) baz
c  version for grid search over baz and phase velocity
c  we eschew the do-loop indentation convention here
c      do ipvel=10,50,10
c      do ibaz=0,350,10
c      baz=float(ibaz)
c      print *,'phase velocity & back azimuth of incident wave'
c      print *,ipvel,ibaz
c      cc=ipvel*1000./vbar
c  end version
...
c      pdelay=0.
c      do i=1,nl
c         sdelay=sdelay+(dz(i)/vs(i))/ren
c         pdelay=pdelay+(dz(i)/vp(i))/ren
c      end do
c      print *, 'organ-pipe mode count at 1 Hz in stack of layers: S & P'
c      print *, 'These are also the S and P travel time from the Moho'
c      print 104,sdelay,pdelay
...
c  calc 3-comp transfer fct response at surface
      ...
      cccc=cc*vbar/1000.
      write(xlabel,1002) cccc
 1002 format(' Freq (Hz) -->  phase velocity',f10.2)
      write(xlabel2,1003) cccc
 1003 format(' Time (sec) -->  phase velocity',f10.2)
      ylabel(1)='Incident P wave'
      ylabel(2)='Incident SV wave'
      ylabel(3)='Incident SH wave'
c  plot responses: P,SV,SH
c      do ity=1,3
c        do k=1,3
c          do i=1,nfrq
c            cc4(i)=zabs(resp(k,ity,i))
c          end do
c          call plotit(frqq,cc4,dum,nfrq,title,xlabel,
c     x      ylabel(ity),2,0,0.05*(3-k),0,(30+ity)*(k/3))
c        end do
c      end do
...
c version for user-supplied cc and baz
      print *,
     x  'wavelet print: 1-onesidedpulse 2-oscillation 3-bbpulse 4-hfbb'
      read(5,*) ipulse
...
c end gridsearch lines   
      if(ipulse.le.2) then
        print *,'enter wavelet period in seconds'
        read(5,*) per
      endif
      t1=0.
      dur=npad*dt
...
c version for usersupplied cc and baz
  960 print *,'tstart, duration to plot?  currently:',t1,t2
      read(5,*) t1,t2
      if(t1.lt.0.0.or.t2.le.0.) go to 900
	if (t2.gt.dur) t2 = dur
...
c note 13 and 4 are the time shift from zero for S and P.	  
c	    if (ity .ne.1) then
c	     tdelay =sdelay - 10.
c	    else
c	     tdelay=pdelay - 10.
c	    endif
c          nst0=tdelay/dt
c let's find the real zero time and shift to +10 sec
          
	    amx = 0.
          do i = 1, npts
	       if (ity .eq. 1) amn=dat44(i,3,ity)
	       if (ity .eq. 2) amn=dat44(i,1,ity)
	       if (ity .eq. 3) amn=dat44(i,2,ity)
...
c comp1=radial comp2=transverse comp3=vert
c  we transform radial & transverse into east & north
          ahead(53)=baz
	    ahead(46)=1. /(cc/1000.*vbar)
	  
c          ahead(53)=baz
c  here we rotate the horizontals 
c  if we are in the loop, BAZ is determined by the loop count
c  if we are in interactive mode, BAZ=0, and the wave is arriving from N
c  if BAZ=0, the wave is arriving from N
c    in anirec.bh? the N component is the -RADIAL
c              and the E component is the TRANSVERSE
c  if BAZ=90, the wave is arriving from E
c    in anirec.bh? the N component is the -TRANSVERSE
c  and the             E component is the -RADIAL
c  if BAZ=180, the wave is arriving from S
c    in anirec.bh? the N component is the RADIAL
c  and the             E component is the -TRANSVERSE
c  if BAZ=270, the wave is arriving from W
c    in anirec.bh? the N component is the TRANSVERSE
c  and the             E component is the RADIAL
          cs=cos(baz* pi / 180.0)
          sn=sin(baz* pi / 180.0)
     ...
     c              cc/1000*vbar,'.',int(baz),'.000',wf(ity)
          do iii=1,18
...
c  write BOTH radial/transverse and north/east horizontal components
c  the radial/transverse SAC files are less vulnerable to BAZ confusion in SAC
...
c            a(i)=dat44(nst0+i-nst,1,ity) +amx *0.01*rand(0)
          end do
          call sacout(name2,a)
...
c	      enddo   
          do i=nst,iah(80)
c            a(i)=dat44(nst0+i-nst,2,ity)+amx *0.01*rand(0)
            a(i)=dat44(nst0+i-nst,2,ity)
          end do
          call sacout(name2,a)
...
c	    end do  
          do i=nst,iah(80)
c            a(i)=dat44(nst0+i-nst,3,ity)+amx*0.01*rand(0)
            a(i)=dat44(nst0+i-nst,3,ity)
          end do
          call sacout(name2,a)
